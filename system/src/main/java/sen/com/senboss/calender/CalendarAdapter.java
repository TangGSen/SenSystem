    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            convertView = LayoutInflater.from(context).inflate(
                    R.layout.item_calendar_grid_view, null, false);
        ...
        if (getItem(position) == -1) {
            ...
            if (calendar.get(Calendar.YEAR) == selectY
                ...
                if (isShowLunar) {
                    //选择农历
                    if (isSelectLunar) {
                        holder.lunarTv.setTextColor(ResourcesUtils.getResColor(context,R.color.colorPrimaryDark));
                        holder.calendarTv.setTextColor(Color.BLACK);
                    } else {
                        holder.calendarTv.setTextColor(ResourcesUtils.getResColor(context,R.color.colorPrimaryDark));
